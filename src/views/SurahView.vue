  ...
  methods: {
    ...
    getArti() {
      axios
        ...
        .then((response) => {
          this.allArti = response.data.translations;
        })
        .catch((error) => {
          console.log(error);
        });
...
<template>
  ...
  <main>
    <div class="container">
      ...
      <div class="my-5">
        <div v-for="(ayat, index) in allAyat" :key="index" class="card">
          <div class="card-body">
            <h5
              class="card-title text-center my-3"
              v-html="ayat.text_uthmani_tajweed"
            ></h5>
            <p class="card-text mt-2">Artinya :</p>
            <p v-html="allArti[index].text"></p>
          </div>
        </div>
      </div>
    </div>
